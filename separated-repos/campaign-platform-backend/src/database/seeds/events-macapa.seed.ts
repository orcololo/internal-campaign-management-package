...
const eventStatuses = ['AGENDADO', 'EM_ANDAMENTO', 'CONCLUIDO', 'CANCELADO', 'ADIADO'] as const;

const macapaLocations = [
...
function generateEventDate(index: number, total: number): Date {
  const now = new Date('2026-01-12'); // Current date (January 12, 2026)
  ...
  if (index < 20) {
    ...
    const timeRange = now.getTime() - sixtyDaysAgo.getTime();
    const offset = timeRange * progress;

    return new Date(sixtyDaysAgo.getTime() + offset);
  }
  ...
  const remainingTotal = total - 20;

  const progress = adjustedIndex / remainingTotal;
  const timeRange = jan31.getTime() - jan1.getTime();
  const offset = timeRange * progress;
...
export const macapaEventsSeed: Partial<NewEvent>[] = Array.from({ length: 50 }, (_, index) => {
  const eventDate = generateEventDate(index, 50);
  const location = randomElement(macapaLocations);
  const type = randomElement([...eventTypes]);

  // Events in the past are more likely to be completed
  const now = new Date('2026-01-12'); // Current date
  const isPast = eventDate < now;
  let status: (typeof eventStatuses)[number];

  if (isPast) {
    const rand = Math.random();
    if (rand < 0.75) status = 'CONCLUIDO';
    else if (rand < 0.9) status = 'CANCELADO';
    else status = 'ADIADO';
  } else {
    status = 'AGENDADO';
  }
  ...
  const title = randomElement(titles[type as string] || titles.OUTRO);

  // Random start times (mostly during business hours and evenings)
  const startHour =
    Math.random() < 0.7 ? Math.floor(Math.random() * 4) + 18 : Math.floor(Math.random() * 6) + 9; // 70% evening, 30% daytime
  const startMinutes = [0, 15, 30, 45][Math.floor(Math.random() * 4)];
  const durationHours =
    type === 'COMICIO' || type === 'DEBATE' ? 3 : Math.floor(Math.random() * 2) + 1;
  ...
  const attendeeRanges: Record<string, string> = {
  ...
  const descriptions: Record<string, string> = {
    COMICIO: `Grande evento de mobilização popular no ${location.neighborhood}. Participação de lideranças locais e apresentação de propostas para a comunidade.`,
    REUNIAO: `Reunião com lideranças comunitárias e empresários do ${location.neighborhood} para discutir demandas locais e propostas de governo.`,
    VISITA: `Visita aos moradores e comerciantes do ${location.neighborhood} para ouvir demandas e apresentar propostas para o bairro.`,
    ...
    CAMINHADA: `Caminhada pelas ruas do ${location.neighborhood} com interação direta com moradores e comerciantes.`,
    CORPO_A_CORPO: `Atividade de corpo a corpo com eleitores no ${location.name}, conversando e ouvindo demandas da população.`,
    EVENTO_PRIVADO: `Evento privado com apoiadores e lideranças para alinhar estratégias e fortalecer a campanha.`,
    OUTRO: `Atividade de campanha no ${location.neighborhood} com foco em mobilização e engajamento da comunidade.`,
  };

  return {
    title,
    description: descriptions[type as string] || descriptions.OUTRO,
    type,
    status,
    ...
    address: `${location.name}, ${location.neighborhood}`,
    city: 'Macapá',
    state: 'AP',
    zipCode: `68900-${Math.floor(Math.random() * 900) + 100}`,
    latitude: location.lat,
    longitude: location.lng,
    expectedAttendees: attendeeRanges[type as string],
    confirmedAttendees:
      status === 'CONCLUIDO' ? String(Math.floor(Math.random() * 200) + 50) : null,
    ...
    notes:
      status === 'CONCLUIDO' ? 'Evento realizado com sucesso. Ótima participação popular.' : null,
    tags: JSON.stringify(
      randomElement([
        ['mobilizacao'],
