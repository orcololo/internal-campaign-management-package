...
export function ReferralsStats({ stats }: ReferralsStatsProps) {
  const favorableCount = stats.byLevel.MUITO_FAVORAVEL + stats.byLevel.FAVORAVEL;
  const favorablePercentage = stats.total > 0 
    ? Math.round((favorableCount / stats.total) * 100) 
    : 0;

  // Calculate growth (comparing thisMonth to average)
  const averageMonthly = stats.total > 0 ? Math.round(stats.total / 6) : 0;
  const growthPercentage = averageMonthly > 0
  ...
  return (
    <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
      ...
      <MetricCard
        ...
        trend={favorablePercentage >= 50 ? "up" : favorablePercentage >= 30 ? "neutral" : "down"}
        icon={Target}
        iconColor="#f59e0b"
      />
    </div>
