...
export default function DashboardPage() {
  ...
  const [previousVoterCount, setPreviousVoterCount] = React.useState<
    number | null
  >(null);
  ...
  const loadDashboardData = async (silent = false) => {
    try {
      ...
      if (
        previousVoterCount !== null &&
        response.data.voters.total > previousVoterCount
      ) {
        const newVoters = response.data.voters.total - previousVoterCount;
        toast.success(
          `${newVoters} novo${newVoters > 1 ? "s" : ""} eleitor${
            newVoters > 1 ? "es" : ""
          } cadastrado${newVoters > 1 ? "s" : ""}!`,
          {
            duration: 3000,
          }
        );
  ...
  // Prepare chart data
  const supportLevelData = Object.entries(
    overview.voters.bySupportLevel || {}
  ).map(([level, count]) => ({
    name: level || "Não definido",
    value: count,
  }));

  const canvassingResultsData = Object.entries(overview.canvassing.results).map(
    ([result, count]) => ({
      name:
        result === "supporters"
          ? "Apoiadores"
  ...
  const voterGrowthData = overview.trends.voterGrowth
    .slice(-30)
    .map((point) => ({
  ...
  const votersGrowth =
    overview.voters.recent.last7Days > 0
      ? (
          (overview.voters.recent.last7Days /
            (overview.voters.total - overview.voters.recent.last7Days || 1)) *
          100
        ).toFixed(1)
      : "0";
          ...
          <div className="grid gap-4 md:grid-cols-2">
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Distribuição de apoio dos eleitores
                </CardDescription>
              </CardHeader>
              <CardContent>
                <ResponsiveContainer width="100%" height={300}>
            ...
            <Card>
              ...
              <CardContent className="space-y-2">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-semibold">
                    {overview.voters.withCoordinates}
                  </span>
                </div>
              </CardContent>
            </Card>
              ...
              <CardContent className="space-y-2">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-semibold text-red-600">
                    {overview.events.cancelled}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span className="text-muted-foreground">Este Mês:</span>
                  <span className="font-semibold">
                    {overview.events.thisMonth}
                  </span>
                </div>
              </CardContent>
            </Card>
            ...
            <Card>
              ...
              <CardContent className="space-y-2">
                <div className="flex justify-between">
                  ...
                  <span className="font-semibold">
                    {overview.canvassing.completedSessions}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span className="text-muted-foreground">Em Andamento:</span>
                  <span className="font-semibold">
                    {overview.canvassing.inProgressSessions}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span className="text-muted-foreground">Total Portas:</span>
                  <span className="font-semibold">
                    {overview.canvassing.totalDoorKnocks}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span className="text-muted-foreground">Taxa Conversão:</span>
                  <span className="font-semibold text-green-600">
                    {overview.canvassing.conversionRate.toFixed(1)}%
                  </span>
                </div>
              </CardContent>
            </Card>
          ...
          <div className="grid gap-4 md:grid-cols-2">
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Top 5 cidades com mais eleitores
                </CardDescription>
              </CardHeader>
              <CardContent>
                <ResponsiveContainer width="100%" height={300}>
                  <BarChart
                    data={Object.entries(overview.voters.byCity || {})
                      .sort(([, a], [, b]) => b - a)
                      .slice(0, 5)
            ...
            <Card>
              ...
              <CardContent>
                <ResponsiveContainer width="100%" height={300}>
                  <PieChart>
                    <Pie
                      data={Object.entries(overview.voters.byGender || {}).map(
                        ([gender, count]) => ({
                      ...
                      dataKey="value"
                    >
                      {Object.entries(overview.voters.byGender || {}).map(
                        (entry, index) => (
                          <Cell
                            key={`cell-${index}`}
                            fill={COLORS[index % COLORS.length]}
              ...
              <div className="grid gap-4 md:grid-cols-2">
                ...
                <div className="flex items-center justify-between p-4 border rounded-lg">
                  <div>
                    ...
                    <p className="text-2xl font-bold">
                      {overview.voters.recent.last30Days}
                    </p>
                  </div>
        ...
        <TabsContent value="canvassing" className="space-y-4">
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Distribuição de resultados das abordagens
              </CardDescription>
            </CardHeader>
            <CardContent>
              <ResponsiveContainer width="100%" height={300}>
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold text-green-600">
                  {overview.canvassing.results.supporters}
                </div>
              </CardContent>
            </Card>
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold text-yellow-600">
                  {overview.canvassing.results.undecided}
                </div>
              </CardContent>
            </Card>
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold text-red-600">
                  {overview.canvassing.results.opponents}
                </div>
              </CardContent>
            </Card>
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold text-gray-600">
                  {overview.canvassing.results.notHome}
                </div>
              </CardContent>
            </Card>
          </div>
        ...
        <TabsContent value="events" className="space-y-4">
          ...
          <Card>
            ...
            <CardContent>
              <ResponsiveContainer width="100%" height={300}>
                <BarChart
                  data={Object.entries(overview.events.byType || {}).map(
                    ([type, count]) => ({
                      type,
                      count,
                    })
          ...
          <Card>
            ...
            <CardContent>
              <ResponsiveContainer width="100%" height={250}>
                <PieChart>
                  <Pie
                    data={Object.entries(overview.events.byStatus || {}).map(
                      ([status, count]) => ({
                        name: status,
                        value: count,
                      })
                    ...
                    dataKey="value"
                  >
                    {Object.entries(overview.events.byStatus || {}).map(
                      (entry, index) => (
                        <Cell
                          key={`cell-${index}`}
                          fill={COLORS[index % COLORS.length]}
