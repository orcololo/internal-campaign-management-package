...
- **Leverage** NestJS decorators for routing, guards, and validation
- **Before finishing tasks**, ensure all code and types are consistent with existing patterns and match implementation between frontend and backend.
...
const active = await db.select().from(voters).where(isNull(voters.deletedAt));
